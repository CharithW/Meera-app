    ...
    return {
        ...
        execute: function() {
            ConferenceComponent = (function () {
                function ConferenceComponent(_meetingService) {
                    this._meetingService = _meetingService;
                    this.room = "Conference";
                    this.subject = null;
                    this.conferenceMeetings = [];
                    this.today = '';
                    this.status = '';
                    this.remainingTime = '';
                    ...
                    this._meetingService.getMeetings()
                        ...
                        meetings.forEach(function (element) {
                            if (((element.location).indexOf("Conference")) !== -1) {
                                _this.conferenceMeetings.push(element);
                            }
                        });
                        var cuurentMeeting = Validations_1.Validations.setVariables(_this.conferenceMeetings);
                        if (Object.keys(cuurentMeeting).length === 0) {
                            var nextMetting = _this.conferenceMeetings[0];
